...
### 🏆 Membership Tiers
| Plan | Watch Limit | Downloads/Day | Video Quality | Price |
|------|-------------|---------------|---------------|-------|
| 🆓 **Free** | 5 minutes | 1 | HD | ₹0 |
| 🥉 **Bronze** | 7 minutes | 3 | Full HD | ₹49/mo |
| 🥈 **Silver** | 10 minutes | Unlimited | 4K | ₹99/mo |
| 🥇 **Gold** | Unlimited | Unlimited | 4K HDR | ₹199/mo |
...
A template is provided in [.env.example](file:///E:/Desktop/StreamNova/.env.example). To configure environment variables locally, copy the example file to `.env.local`:
...
The app works out of the box with the default local mock configuration, but you can update `.env.local` with real values (such as Razorpay credentials or SMTP details) to enable production-level features.
...
| **Context API** | Global state management (auth, theme, plan) |
| **WebRTC** | Real-time peer-to-peer video/audio in Live Room |
| **jsPDF + html2canvas** | Client-side PDF invoice generation |
| **react-icons** | Feather icon set throughout the UI |
| **Razorpay SDK** | Payment gateway (mock mode in development) |
...
|------------|--------|----------------|
| `≥ 1280px` | Desktop | Full 4-column plan grid, 3-column video grid |
| `1024–1280px` | Laptop | 2-column plan grid |
| `768–1024px` | Tablet | Single-column watch layout, stacked profile |
| `480–768px` | Phone | Hamburger menu, 2-column video grid |
| `< 480px` | Small phone | 1-column video grid, bottom-sheet modals |
...
|------|-------|-------------|
| **Home** | `/` | Hero, video grid, category filter, feature showcase |
| **Watch** | `/watch/[id]` | Cinema player + multilingual comments + recommendations |
| **Premium** | `/premium` | Plan cards, billing toggle, comparison table, FAQ |
| **Live Room** | `/room` | WebRTC video grid, controls, YouTube co-watch |
| **Profile** | `/profile` | User info, plan badge, download history |
| **Auth** | `/auth` | Email/Phone OTP login flow |
